...
class AddOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        return [output_grad * 1, output_grad * 1]
...
class AddByConstOp(Op):
    ...
    def compute(self, node, input_vals):
        """Given values of input nodes, return result of element-wise addition."""
...
class Executor:
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            # we have nothing to do with placeholder nodes
            if isinstance(node.op, PlaceholderOp):
                continue
            vals = [node_to_val_map[i] for i in node.inputs]
            computed_val = node.op.compute(node, vals)
            if isinstance(computed_val, np.ndarray):
                node_to_val_map[node] = computed_val
            else:
                node_to_val_map[node] = np.array(computed_val)
            print(node, node_to_val_map[node], sep=':\n')
...
def gradients(output_node, node_list):
    ...
    # We are actually taking a derivative of the scalar reduce_sum(output_node)
    ...
    for node in reverse_topo_order:
        ...
        for i, incoming_node in enumerate(node.inputs):
            grads = node.op.gradient(node, grad)
            grads_list = node_to_output_grads_list.get(incoming_node, [])
            grads_list.append(grads[i])
            node_to_output_grads_list[incoming_node] = grads_list
...
def sum_node_list(node_list):
    ...
    return reduce(lambda a, b: a + b, node_list)
